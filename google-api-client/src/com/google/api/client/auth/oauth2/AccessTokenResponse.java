 ...
 * href="http://tools.ietf.org/html/draft-ietf-oauth-v2-07#section-5.1.5" >Access Token
 * Response</a>.
 * <p>
 * Use {@link AccessProtectedResource} to authorize executed HTTP requests based on the
 * {@link #accessToken}, for example {@code
...
public class AccessTokenResponse extends GenericData {
  ...
  /**
   * (OPTIONAL) The refresh token used to obtain new access tokens using the same end-user access
   * grant.
   */
  @Key("refresh_token")
  ...
  /**
   * (OPTIONAL) The scope of the access token as a list of space-delimited strings. The value of the
   * "scope" parameter is defined by the authorization server. If the value contains multiple
